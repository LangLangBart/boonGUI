...
class Debug
{
	...
	slow(...args)
	{
		if (g_LastTickTime - this._lastWarn >= 2000)
		{
			this._lastWarn = g_LastTickTime;
			warn(uneval([...args]));
		}
	}
	...
	{
		this._timers[name] = { "start": Engine.GetMicroseconds() };
	}
	...
	{
		const end = Engine.GetMicroseconds();
		warn(`${name}: ${(end - this._timers[name].start) / 1000}ms`);

...
// register global fuctions for the Simulation
...
// The Simulation can't access the "Engine.GetMicroseconds()" function
global.timeTaken = (callback, count = 1) => {
	...
	for (let i = 0; i < count; i++)
	{
		...
		callback();
		measure.push(Date.now() - startTime);
	}
	const average = measure.reduce((acc, val) => acc + val, 0) / count;
	warn(`${average}ms ${count > 1 ? measure : ""}`);
};
